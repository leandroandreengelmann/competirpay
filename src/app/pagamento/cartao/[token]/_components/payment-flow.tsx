...
import { processPayment, getPaymentStatus } from "../actions";
...
export function PaymentFlow({ token, initialData }: PaymentFlowProps) {
    const [status, setStatus] = useState<string>(initialData.status);
    const [attemptStatus, setAttemptStatus] = useState<string>("READY");
    const [isLoading, setIsLoading] = useState(false);
    const [globalError, setGlobalError] = useState<string | null>(null);

    // Derive the active view state
    const isFormActive = status === "ACTIVE" && attemptStatus === "READY";
    const isTerminalState =
        status === "EXPIRED" || status === "USED" || status === "CANCELLED" ||
        attemptStatus === "APPROVED" || attemptStatus === "REFUSED" || attemptStatus === "ERROR";

    const showStatusOnly = !isFormActive;

    // Polling logic
    useEffect(() => {
        let intervalId: NodeJS.Timeout;

        if (attemptStatus === "PROCESSING" || attemptStatus === "PENDING_CONFIRMATION") {
            intervalId = setInterval(async () => {
                try {
                    const res = await getPaymentStatus(token);
                    if (res.success) {
                        if (res.linkStatus !== status) setStatus(res.linkStatus);
                        if (res.attemptStatus !== attemptStatus) setAttemptStatus(res.attemptStatus);
                    }
                } catch (e) {
    ...
    const handleSubmit = async (formData: PaymentFormData) => {
        ...
        try {
            const res = await processPayment(token, formData);
            if (!res.success) {
                setGlobalError(res.error || "Ocorreu um erro ao processar o formulário.");
                setIsLoading(false);
            } else {
                // If success, the server already created an attempt as PROCESSING or similar.
                setAttemptStatus(res.data?.status || "PROCESSING");
                // Start polling via useEffect
                setIsLoading(false);
            }
        } catch (err) {
            setGlobalError("Erro na comunicação com o servidor.");
            setIsLoading(false);
        }
    };

    // Determine which status to show based on link vs attempt hierarchy
    const statusToShow =
        status === "EXPIRED" ? "EXPIRED" :
            status === "CANCELLED" ? "CANCELLED" :
                status === "USED" && attemptStatus === "READY" ? "USED" : // Used previously but no new attempt 
                    attemptStatus !== "READY" ? attemptStatus : // PROCESSING, APPROVED, REFUSED
                        "READY";

    return (
        <div className="w-full max-w-3xl mx-auto p-4 sm:p-6 md:p-8 flex flex-col items-center">
            {/* Header Layout */}
            <div className="mb-8 mt-4 text-center">
                <h1 className="text-3xl font-extrabold tracking-tight mb-2">Pagamento com Cartão</h1>
                <p className="text-muted-foreground">Finalize o pagamento da sua solicitação</p>
            </div>
            ...
            <div className="w-full">
                {isFormActive ? (
                    <>
                        <PaymentSummary
                            name={initialData.credit_requests.name}
                            championshipName={initialData.credit_requests.credit_tables?.name || "Campeonato"}
                            totalAmount={initialData.amount}
                            installments={initialData.installments}
                            paymentMethod={initialData.credit_requests.payment_method}
                        />
